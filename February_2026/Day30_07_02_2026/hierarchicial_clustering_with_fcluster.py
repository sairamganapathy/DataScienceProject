...
for i, method in enumerate(methods, 1):
    ...
    # --- Use fcluster to get labels ---
    # Here we say: cut tree into exactly 3 clusters
    labels = fcluster(Z, t=3, criterion="maxclust")

    # Plot scatter to show clusters formed
    plt.figure()
    plt.scatter(X[:, 0], X[:, 1], c=labels, cmap="rainbow", s=60, edgecolor="k")
    plt.title(f"{method.capitalize()} Linkage Clusters (fcluster)")
